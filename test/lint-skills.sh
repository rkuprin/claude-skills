...
has   "reference: recap renders each finding"   "Critic's findings:"   "$ORCH_REF"
has   "reference: recap names the end state"    "End state:"           "$ORCH_REF"
has   "reference: recap is screen-self-contained" "opens nothing else" "$ORCH_REF"
has   "constitution: operator surface section"  "## The Operator Surface" "$ORCH"
has   "constitution: screen is operator surface" "the screen is the operator's surface" "$ORCH"
has   "constitution: path never carries the decision" "never the carrier" "$ORCH"
has   "constitution: questions asked like out loud"  "a person in a" "$ORCH"
has   "constitution: question needs nothing opened" "not ready to ask" "$ORCH"
has   "reference: recap questions self-contained"  "answerable without opening anything" "$ORCH_REF"
has   "constitution: brief is a dialogue"          "a dialogue, not an intake form" "$ORCH"
has   "constitution: one question at a time"       "one question at a time" "$ORCH"
has   "constitution: play back understanding"      "only the operator tells you why it matters" "$ORCH"
has   "constitution: product questions asked live" "never banked for the" "$ORCH"
has   "constitution: brainstorming lineage with carve-out" "development machinery and never apply here" "$ORCH"
has   "reference: recap questions are leftovers"   "not banked for this line" "$ORCH_REF"
...
has   "reference: claude watch at idle budget"     "watch <sprint-dir> '*-question.md *-concluded.md' 10800" "$ORCH_REF"
...
has   "handoff: claude wait form is the background watch" "sprint-mail.sh watch '{SPRINT_DIR}' '{NN}-{SSS}-reply.md' 1800" "$AH"
...
has   "contract: claude main-session watch wait" "sprint-mail.sh watch <sprint-dir> '{NN}-{SSS}-reply.md' 1800" "$AHEXEC"
...
has   "renderer: claude wait is the background watch" "sprint-mail.sh watch" "$WHS"
...
# The Claude Stop hook is retired — the Codex hook is the only Stop-hook wait.
hasnt "stop-wait: codex hook is the only stop hook" "claude-stop-wait" "$CSW"
has   "mail: arm usage line"                    "arm --harness <codex> <sprint-dir> <name-or-glob(s)>" "$SMAIL"
...
has   "mail: arm refuses claude with redirect"  "background watch, not a Stop hook" "$SMAIL"
...
has   "sprint readme: claude migration note"      "delete the \`claude-stop-wait.sh\`" "$ORCH_README"
...
has   "repo readme: claude needs no hook" "Claude and Kimi need no hook" "$HERE/../README.md"
...
hasnt "install guide: no claude hook installer" "install-claude-hook.sh" "$INSTALL"
